...
function handleAnnot(annot, testcase) {
  ...
  switch (type) {
    ...
    case "verovio-options":
      if (Object.keys(testcase.verovioOptions).length > 0) {
        testcase.errors.push("File must not contain multiple verovio-options annotations");
        break;
      }
      try {
...
const randomXmlName = (function() {
  ...
  // it. We also exclude "." because it makes working with CSS attribute
  // selectors more complicated.
...
const defaultVerovioOptions = {breaks: "encoded", svgHtml5: true};
...
function constructVerovioArgs(testcase) {
  ...
  for (const [key, value] of Object.entries(options)) {
    ...
    if (key === "engravingDefaults" && typeof value === "object") {
      // We want to make engraving defaults values accessible to the test test
      // code. We don't want to duplicate the values, so we allow a JSON
      // structure instead of a path under the option "--engraving-defaults".
      const jsonPath = testcase.svgPath.replace(/svg$/, 'json');
      fs.writeFileSync(jsonPath, JSON.stringify(value));
      args.push("--engraving-defaults");
      args.push(jsonPath);
    } else if (value !== false) {
